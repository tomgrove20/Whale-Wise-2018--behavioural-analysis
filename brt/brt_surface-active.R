...
fol.foc <- read.csv("intermediate-products/follows/follows+foc+ais+bath_2018-20_calc.csv") %>%
  mutate(direction.time.a = as.factor(direction.time.a),
         `log(dist)` = log(dist),
         eng.off.on.1sur = as.factor(eng.off.on.1sur),
         surface.active = as.numeric(surface.active))
...
nrow(fol.foc %>% filter(surface.feeding == 1)); nrow(fol.foc %>% filter(surface.feeding == 0))
...
ts = c(10,60,300)
var.s = c("groupsize", "direction.time.a", "log(dist)", "beaufort", "day.of.year", "encounter.minute", "eng.off.on.1sur")
var.t = c("focal.vess.speed", "focal.vess.accel", "focal.vess.di")
...
for (i in 1:nrow(t.df)) {
  ...
  v <- vars %>% dplyr::recode(!!t.df[i,"var.orig"] := t.df[i,"var.target"]) # variable list
  
  indices <- vector() ; for(j in v){indices = c(indices, grep(paste0("^",j,"$"), colnames(tot)))} # column indices
  
  # run the gbm
  set.seed(0)
  t <- gbm.step(data = tot, gbm.x = indices, gbm.y = 108, family = "bernoulli",
                tree.complexity = 2, n.trees = 500, max.trees = 500,
                learning.rate = 0.01, bag.fraction = 0.75) # tc = 2: 2D interactions
...
# and run again ...
indices <- vector() ; for(i in vars){indices = c(indices, grep(i, colnames(tot)))}
t <- gbm.step(data = tot, gbm.x = indices, gbm.y = 1, family = "gaussian", max.trees = 1500,
              tree.complexity = 2,
              learning.rate = 0.01, step.size = 100,
              bag.fraction = 0.75,
              keep.data = TRUE)
...
gbm.plot(t, smooth = TRUE, rug = FALSE, y.label = "marginal effect", common.scale = FALSE, plot.layout = c(4,3), lwd = 2)

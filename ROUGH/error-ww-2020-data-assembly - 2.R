...
follow <- read.csv("data/error/20200818_saeborg_error_follows.csv") %>%
  full_join(read.csv("data/error/20200818_saeborg_error_follows.csv"))
...
vess <- read.csv("data/error/20200818_1615_error_saeborg_gps.csv", fileEncoding="UTF-16LE", sep = "\t", header = TRUE) %>%
  rename(datetime = time, lat = Latitude, lon = Longitude) %>%
  select(datetime, lat, lon) %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S"))
head(vess)

# we can see there are missing seconds here. This will present a problem when matching with follow data. We thus need to populate missing seconds with locations from either side (should be fine as Saeborg was stationary)

secs <- as.data.frame(seq.POSIXt(min(vess$datetime), max(vess$datetime), by = "sec")) %>%
  rename(datetime = 1) # creating list of every second from start to finish

vess <- full_join(vess, secs) # adding rows for missing seconds
...
whale <- read.table("data/error/20200818_1615_error_inflatable_gps.csv", fileEncoding="utf-16", sep = "\t", header = TRUE) %>%
  rename(datetime = time, lat = Latitude, lon = Longitude) %>%
  select(datetime, lat, lon) %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S")) %>%
  st_as_sf(coords = c("lon", "lat"), crs = "EPSG:4326", remove = FALSE) %>%
...
write.table(follow.tot, "intermediate-products/error/error_raw-tot_20200818_saeborg.txt", sep="\t")

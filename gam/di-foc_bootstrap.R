# testing bootstrapping model results (speed + DI)
...
tot <- tot %>%
  mutate(arcsin.DI = asin(DI),
         across(contains(c("difftime","vess.speed.var")), .fns = list(log = ~log(.)), .names = "{fn}.{col}"),
...
# run!
di.foc.boot <- foreach(i = 1:it, .combine = rbind,.errorhandling = 'remove', .packages = c("tidyverse", "mgcv")) %dopar% {
...
write.csv(di.foc.boot, "intermediate-products/gam/di/foc/di_foc_boot-res.csv", row.names = FALSE)
...
#di.foc.boot <- read.csv("intermediate-products/gam/di/foc/di_foc_boot-res.csv")

# first a df for naming
var.fac = c("grouplone", "seastatechoppy")
var.num = c("day.of.year", "sqrt.dist", "log.difftime.prev", "log.difftime.next", "sqrt.vess.speed.60",  "log.vess.speed.var.300", "arcsin.vess.di.300", "vess.accel.max.60")
name.df <- data.frame(
  var = c(var.fac, var.num),
  name = c("Group size", "Sea state", "Julian day", "Distance", "IBI before", "IBI after", "Vessel speed (60 sec.)", "Vessel speed SD (300 sec.)", "Vessel DI (300 sec.)", "Max vessel acceleration (60 sec.)")) %>%
...
plot_grid(
  plot_grid(pfac,NULL, ncol = 1, rel_heights = c(1.3,3)),
  pnum)
ggsave("intermediate-products/gam/di/foc/di_foc_boot_res.png", height = 14, width = 7)

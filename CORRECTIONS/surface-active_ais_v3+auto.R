...
f <- as.formula(paste("surface.active ~", 
                      paste("s(",var.int,", bs = 'cp', k = 3)", collapse = "+"),"+",
                      paste("s(",var.num,", bs = 'cp', k = 10)", collapse= "+"),"+",
...
ggsave("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_acf.png", width = 11, height = 3.6)
...
gam1 <- do.call("bam", list(as.formula(f), data=as.name("tot"), method = "fREML", family = binomial,
                            rho = 0.3, AR.start = ar.start, discrete = TRUE))
...
gam <- do.call("bam", list(as.formula(f), data=as.name("tot"), method = "fREML", family = binomial,
                            rho = 0.3, AR.start = ar.start, discrete = TRUE))
summary(gam); AIC(gam) # dev = 50.8%, fREML = 6622.8, AIC = 824.96

# final model after manual removal (AIC + dev.ex, supplemented by fREML score)
var.fac = c("group")
var.int = c("rib.num.30.1500")
var.num = c("sqrt.oak.meandist.30.1500", "sqrt.rib.meandist.30.1500")
var.rand = c("folnum.unique")
...
gam <- do.call("bam", list(as.formula(f), data=as.name("tot"), method = "fREML", family = binomial,
                           rho = 0.3, AR.start = ar.start, discrete = TRUE))
summary(gam); AIC(gam) # dev = 50.9%, fREML = 6625.7, AIC = 809.3

# that is the final model! Let's check for concurvity
concurvity(gam); concurvity(gam, full = FALSE) # only high concurvity between folnum.unique and other vars

# saving the final model!
gam.final = gam; saveRDS(gam.final, "intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_final_v3.rds")


#---------------- MODEL RESULTS --------------------
# gam <- readRDS("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_final_v3.rds")

# first saving estimates
res <- bind_rows(as.data.frame(summary(gam)$p.table), as.data.frame(summary(gam)$s.table)) %>%
  rownames_to_column("var")
write.csv(res, "intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_results_v3.csv", row.names = FALSE)
...
var.fac = c("group")
var.num = c("rib.num.30.1500", "sqrt.oak.meandist.30.1500", "sqrt.rib.meandist.30.1500")
var.rand = c("folnum.unique")

# oak.num.30.1500
(sm <- plot(sm(b, 1)) + l_fitLine() + l_ciLine() + l_rug() + labs(x = "RIB boats", y = "s(RIB boats)") + partial.theme +
    partial.grid.margin + scale_x_continuous(breaks = seq(0,10,2)))
(pr <- pred.var(gam, tot, resp = "surface.active", var = "rib.num.30.1500", var.fac, var.num, var.rand) %>% 
    ...
    labs(x = "RIB boats", y = "Rate of surface activity") + partial.theme + partial.grid.margin +
    scale_x_continuous(breaks = seq(0,10,2)))
(comb <- as_ggplot(gridPrint(sm, pr, ncol = 2)) + 
    draw_plot_label(label = c("A", "B"), x = c(0, 0.5), y = c(1, 1), size = 20))
ggsave("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_rib.num.30.1500_v3.png", height=4.5, width=13)

# sqrt.oak.meandist.30.1500
  (sm <- plot(sm(b, 2)) + l_fitLine() + l_ciLine() + l_rug() + labs(x = "sqrt(oak boat mean distance travelled (m))", y = "s(sqrt(oak boat mean distance))") + partial.theme +
    partial.grid.margin)
(pr <- pred.var(gam, tot, resp = "surface.active", var = "sqrt.oak.meandist.30.1500", var.fac, var.num, var.rand) %>% 
    ggplot(aes(x = (sqrt.oak.meandist.30.1500)^2, y = predict)) + geom_line() +
    geom_ribbon(aes(ymin = lwr, ymax = upr), linetype = "dashed", color = "black", fill = "transparent") +
    scale_x_continuous(trans = "sqrt", breaks = c(50,200,500,1000,2000, 5000, 10000)) +
    labs(x = "Oak boat mean distance travelled (m)", y = "Rate of surface activity") + partial.theme + partial.grid.margin)
(comb <- as_ggplot(gridPrint(sm, pr, ncol = 2)) + 
    draw_plot_label(label = c("A", "B"), x = c(0, 0.5), y = c(1, 1), size = 20))
ggsave("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_oak.meandist.30.1500_v3.png", height=4.5, width=13)

# sqrt.rib.meandist.30.1500
(sm <- plot(sm(b, 3)) + l_fitLine() + l_ciLine() + l_rug() + labs(x = "sqrt(RIB mean distance travelled (m))", y = "s(sqrt(RIB mean distance))") + partial.theme +
    partial.grid.margin)
(pr <- pred.var(gam, tot, resp = "surface.active", var = "sqrt.rib.meandist.30.1500", var.fac, var.num, var.rand) %>% 
    ...
    geom_ribbon(aes(ymin = lwr, ymax = upr), linetype = "dashed", color = "black", fill = "transparent") +
    labs(x = "RIB mean distance travelled (m)", y = "Rate of surface activity") + partial.theme + partial.grid.margin )
(comb <- as_ggplot(gridPrint(sm, pr, ncol = 2)) + 
    draw_plot_label(label = c("A", "B"), x = c(0, 0.5), y = c(1, 1), size = 20))
ggsave("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_rib.meandist.30.1500_v3.png", height=4.5, width=13)
...
(comb <- as_ggplot(gridPrint(sm, pr, ncol = 2)) + 
    draw_plot_label(label = c("A", "B"), x = c(0, 0.5), y = c(1, 1), size = 20))
ggsave("intermediate-products/gam-v3/surface-active/ais/gam_surface-active_ais_group_v3.png", height = 4.5, width = 13)
